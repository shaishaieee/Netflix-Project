...
const ModalSearch = ({setSearch}) => {
  ...
  return (
    ...
    <div className="relative p-2">
        <input type="text" className=' outline-none focus:border-accent w-full border-b border-stone-500 px-4 py-3 bg-transparent placeholder:opacity-10' placeholder='Search. . .' onChange={(e) => handleKeyword(e)}/>
        <button className='absolute top-8 right-6' onClick={getSearchResult}><FaSearch/></button>
    </div>

    {result.length !== 0 ? (<div className="result p-3">
        <h3 className='font-bold mb-5 text-[#232323]'>Results keyword "<span className='text-white'>{keyword}</span>"</h3>
